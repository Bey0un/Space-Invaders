...
int init_score()//
{
    texte_surface = TTF_RenderText_Solid(police, "SCORE  0", couleur_score);
    return 0;
}
...
{   
    ...
    SDL_BlitSurface(texte_surface, NULL, surface, &position_texte);
    char texte_score[100];
    sprintf(texte_score, "SCORE  %d", score);
    texte_surface = TTF_RenderText_Solid(police, texte_score, couleur_score);
}
